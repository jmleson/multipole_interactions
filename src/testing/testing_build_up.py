...
from src.term_components.Index import Index
from src.term_components.Multipole import MultipoleMoment
from src.build_up_tensor_equations.construct_full_tensor import construct_all_tensor_terms, r_part
from src.build_up_tensor_equations.tensor import TensorTerm, Tensor
from src.build_up_tensor_equations.variations_between_R_and_delta import variations_between_R_and_delta
...
class TestAuildUp(unittest.TestCase):
    ...
    def test_tensor_to_equation(self):
        ...
        quadrupole.molecule = "A"
        t = TensorTerm(m1=dipole, m2=quadrupole)
        assert t.order == 4
        assert t.get_prefactor() == sp.sympify("1/9")
        assert "+ 1/9 * T_αβγδ * Θ^B_αβ * Θ^A_γδ" == t.to_string()
    ...
    def test_get_product_terms_from_greek_sum_start(self):
        p = ProductTerm()
        ...
        p = ProductTerm()
        p.set_elements([MultipoleMoment(["alpha", "alpha", "alpha"])], prefactor=1)
        result = get_product_terms_from_greek_sum_duplicateMultipoles(p)
        assert len(result) == 1
        assert result[0] == p
        ...
        for x in ["x", "y", "z"]:
            for y in ["x", "y", "z"]:
                for z in ["x", "y", "z"]:
                    s = "".join(sorted([x, y, z]))
                    count += 1
                    assert f"+ 1 * Ω^B_{s} * Ω^A_{s}" in strings
        assert len(result) == count


        result = get_p_result(["alpha", "beta", "z"], ["alpha", "beta", "z"])
        strings = [i.to_string() for i in result]
        count = 0
        for x in ["x", "y", "z"]:
            for y in ["x", "y", "z"]:
